//
//  ShimmersMacros/Macros/Formals/Assume Macro.swift
//  shimmers-hdl
//
...
struct AssumeMacro: ExpressionMacro {
    static func simpleDecodeParam(
        ...
        var type: ExprSyntax = ".assumption"
    ...
    static func decodeParam(
        ...
        guard let arg1 = arguments.last, arg1.label == nil else {
            throw MacroExpansionErrorMessage("Missing assumed value argument")
        }
        value = arg1.expression
        arguments.removeLast()
        ...
        if let arg = arguments.last {
            throw MacroExpansionErrorMessage("Extra extra argument '\(arg.trimmedDescription)'")
        }
    ...
    static func expansion(
        ...
        if !checkHardwareParent(in: context) {
            let warning = MacroExpansionWarningMessage("'#assumed' can be replaced by regular assert outside of '@HardwareWire' struct")
        ...
        return "Swift.precondition(\(value.trimmed), \(message ?? "\"Simulation assumption violated.\"")) // type: \(raw: type.trimmed)"
    }
}
