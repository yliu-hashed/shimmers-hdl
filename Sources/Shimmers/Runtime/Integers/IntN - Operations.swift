//
//  Shimmers/Runtime/Integers/IntN - Operations.swift
//  shimmers-hdl
//
...
public extension IntN {
    ...
    static func &<<= <RHS>(lhs: inout Self, rhs: RHS) where RHS : BinaryInteger {
        lhs.value <<= rhs
        lhs.value.truncatingSigned(to: Self.bitWidth)
    }
...
public extension IntN {
    func addingReportingOverflow(_ rhs: Self) -> (partialValue: Self, overflow: Bool) {
        ...
        if result > Self.bigMax || result < Self.bigMin {
            result.truncatingSigned(to: Self.bitWidth)
    ...
    var magnitude: UIntN<__bitWidth> {
