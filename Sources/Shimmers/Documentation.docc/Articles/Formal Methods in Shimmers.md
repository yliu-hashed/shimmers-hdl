...
Take advantage of Shimmers' built-in formal tools.
...
There are many places where Swift crashes at runtime, such as integer overflow.
...
* Integer arithmetic (like `+`, `-`, `*`)
* Integer conversion (like `UInt8(1234567)`)
...
Doing this not only avoids overflow checks but also tells the reader that a wrapping addition is supposed to happen.
...
The addition of assertions will ensure that the circuit is always correct by proving that the input of a circuit is of a restricted domain.
...
The ``assume(_:type:_:)`` macro is used to establish the premise. 
...
This is done by unrolling one step at a time until the condition must be false.
...
> Warning:
It is your responsibility to make sure that any input into a hinted loop never exceeds the hinted upper bound.
...
This allows any type of macros to be disabled during synthesis.
