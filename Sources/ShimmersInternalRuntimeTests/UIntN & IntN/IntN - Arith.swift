...
struct IntNArithmeticsTestSuite {
    ...
    @Test func intN_conversions() async {
        for i in 0..<count {
            let result = vals[i]
            let truth  = refs[i]
            #expect(result == truth)
