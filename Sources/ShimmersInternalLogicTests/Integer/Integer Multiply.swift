//
//  ShimmersInternalLogicTests/Integer/Integer Multiply.swift
//  shimmers-hdl
//
...
@HardwareWire
fileprivate struct UnsignedMultiplier {
    var result: UInt8
    var overflow: Bool

    static func mul(_ a: UInt8, _ b: UInt8) -> Self {
...
@HardwareWire
fileprivate struct SignedMultiplier {
    var result: Int8
    var overflow: Bool

    static func mul(_ a: Int8, _ b: Int8) -> Self {
        let (result, overflow) = a.multipliedReportingOverflow(by: b)
        return .init(result: result, overflow: overflow)
    }
}
...
struct IntegerMultiplicationTestSuite {
    @Test func multiplication_unsigned() async throws {
        ...
        for a in values {
            for b in values {
                let answer = a.multipliedReportingOverflow(by: b)
